...
export class OrdersService {
  ...
  async updateStatus(orderId: string, next: OrderStatus) {
    ...
    if (next === OrderStatus.CANCELLED) {
      ...
      if (!cancellableStatuses.includes(order.status)) throw new AppException('INVALID_STATUS_TRANSITION', 'Este pedido não pode mais ser cancelado.', HttpStatus.CONFLICT);
